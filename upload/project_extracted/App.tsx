...
import { ProcessedImage, ProcessStatus, LogoSettings, ProcessingMode, AiModel, SlicedResult, SlicedPiece, LogoItem, BatchResult, FolderNode } from './types';

// Memory-safe zip generation helper — streams output to avoid OOM on large batches
...
import { applyLogoToImage, getImageDimensions, applyGlobalCrop, loadElementImage } from './services/canvasService';
import { LogoControls } from './components/LogoControls';
import { CropModal } from './components/CropModal';
import { CustomSliceModal } from './components/CustomSliceModal';
import { DetailSlicePreview } from './components/DetailSlicePreview';
import { sliceImage } from './services/sliceService';
import { saveSettings, loadSettings, saveImages, loadImages, saveLogos, loadLogos, saveBatchResults, loadBatchResults, saveSlicedResults, loadSlicedResults, clearAllStorageData } from './services/storageService';
...
function App() {
  ...
  const [batchCropAspect, setBatchCropAspect] = useState<number | null>(1); // 1 = 1:1, 0.75 = 3:4, null = custom
  ...
  const [currentFolderPath, setCurrentFolderPath] = useState<string | null>(null); // null = root
  ...
  const scheduleSave = useCallback(() => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      if (isProcessingRef.current) return; // don't save mid-processing
      try {
        saveSettings({
  ...
  useEffect(() => {
    const handleHide = async () => {
      ...
      try {
        ...
        // Use synchronous-style fire-and-forget for unload events
        saveImages(imagesRef.current);
        saveLogos(logoItems);
        saveBatchResults(batchResults);
        saveSlicedResults(slicedResults);
  ...
  const currentFolderImages = useMemo(() => {
    if (!currentFolderPath) {
      // Root: show images without relativePath only
      return images.filter(img => !img.relativePath);
    }
    // Show images whose relativePath exactly matches currentFolderPath
  ...
  const getRelativePath = (file: File): string => {
    ...
    if (rel && rel.includes('/')) {
      // e.g. "myFolder/sub/photo.jpg" → "myFolder/sub"
      return rel.substring(0, rel.lastIndexOf('/'));
    }
    return '';
  };

  // Unified File Handler — supports both loose files and folder files
  // Uses async chunked loading to avoid blocking the main thread for large batches
  const handleFiles = async (files: FileList | File[]) => {
    ...
    // Chunk size for async loading — prevents main thread freeze on 1000+ files
    const CHUNK_SIZE = 50;
    const chunks: File[][] = [];
  ...
  // Handle Main Image Upload (Input) — supports both files and folder
  ...
  // Handle Folder Upload (Input) — webkitdirectory attribute
  ...
  // Drag and Drop Handlers — supports dropping files AND folders
  ...
  // Recursively traverse FileSystemDirectoryHandle to collect all image Files
  const traverseDirectoryEntry = async (entry: any, path: string = ''): Promise<File[]> => {
    ...
    if (entry.isFile) {
      ...
      // Attach the relative path info for directory structure replication
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (!items || items.length === 0) {
      // Fallback: no DataTransferItemList support
  ...
  // Handle Logo Upload (multi-file) — filter out hidden/system files
  ...
  const processBatch = async () => {
    ...
    // Revoke old batch result blob URLs to prevent memory leak on re-process
    ...
    // ── Phase 1: Pre-process all images (crop + AI) ──
    for (const imageId of imageIds) {
      ...
      try {
        ...
        // Revoke previous resultUrl if it was a blob URL
    ...
    if (shouldAddLogo && logoItems.length > 0 && preprocessedMap.size > 0) {
      ...
      // Build a lookup map for O(1) access to result entries by logoId+imageId
      ...
      // Pre-load all logo images to avoid redundant loading
      ...
      await Promise.all(logoItems.map(async (logo) => {
        ...
        } catch { /* ignore, will fail per-result */ }
      }));
  ...
  // Download Selected Handler — preserves subfolder structure in ZIP
  ...
  // Download individual batch result
  ...
  // Download batch results by logo and optional folder path
  ...
  // ── Folder Download: download all completed images in a folder as ZIP ──
  const downloadFolderImages = async (folderPath: string, folderName: string) => {
    // Collect all completed images that belong to this folder (including subfolders)
  ...
  // Download all batch results as zip — replicates directory structure per Logo
  const handleDownloadAllBatchResults = async () => {
    ...
    try {
      ...
      await Promise.all(completed.map(async (result) => {
        ...
        // Build path: logoName/[originalFolderPath]/imageName.png
  ...
  const handleSliceImage = useCallback(async (imageId: string) => {
    ...
    // Manual mode: open CustomSliceModal
  ...
  // Handle custom slice crop result from CustomSliceModal
  ...
  // Determines the image to show in the logo editor preview
  ...
  // ── Batch results grouping & navigation ──
              ...
              {currentFolderNode && currentFolderNode.children.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-4 mb-6">
                  {currentFolderNode.children.map((folder) => {
                    // Count completed images in this folder (recursively)
